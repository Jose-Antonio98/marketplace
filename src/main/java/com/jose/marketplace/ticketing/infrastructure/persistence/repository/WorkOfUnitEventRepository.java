package com.jose.marketplace.ticketing.infrastructure.persistence.repository;

import com.jose.marketplace.ticketing.domain.*;
import com.jose.marketplace.ticketing.infrastructure.persistence.entity.SeatLock;
...
public class WorkOfUnitEventRepository implements EventRepository {
    ...
    public void save(Event event) {
        var sectors = event.getSeats().entrySet().stream()
                .map(entry -> {
                    ...
                    var seats = domainSeats.stream()
                            .map(s -> new com.jose.marketplace.ticketing.infrastructure.persistence.entity.Seat(
                                    s.getId(),
                                    s.getCorrelationId().id()
                            ))
                            .toList();

                    return new com.jose.marketplace.ticketing.infrastructure.persistence.entity.Sector(
                            domainSector.getId(),
        ...
        var entity = new com.jose.marketplace.ticketing.infrastructure.persistence.entity.Event(
                event.getId(),
